...
class ExercisePageView extends StatefulWidget {
  ...
  ExercisePageView({this.title, this.snapshot, this.auth});
}
...
class _ExercisePageViewState extends State<ExercisePageView>
  ...
  void initState() {
    ...
    _index = (widget.snapshot.data['exercises'].length + 1);
  ...
  Widget animatedHangboardPage(int index) {
    ...
    /*if (index == _currentPageValue.floor()) {
      ...
      return HangboardPage(
        index: index,
        exerciseParameters:
            Map<String, dynamic>.from(widget.snapshot.data['exercises'][index]),
      );
    }
  }
